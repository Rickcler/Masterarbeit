...
Let $(X_t)$ be a \textit{stationary categorical time series}, where each $X_t$ takes values in
\[
S = \{s_0, \dots, s_m\}, \qquad m \in \mathbb{N}.
\]
$(X_t)$ may be \text{nominal}, in which case the order of the elements in $S$ in arbitrary and solely useful for notational consistency, or $ordinal$, in which case the order of the elements in $S$ reflects the inherent oder     of the elements.  
...
Since ordinal and nominal scales encode different types of information, their marginal distributions require different representations.
