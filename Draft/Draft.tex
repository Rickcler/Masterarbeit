...
The choice of a location measure for a categorical process depends on whether the process is nominal or ordinal. 
...
For the estimation of the (cumulative) probabilities based on samples we use the (cumulative) relative frequencies, that can best be expressed by switching from $(X_t)$ to its binarization ($(\boldsymbol{Z_t})$ for ordinal variables, $(\boldsymbol{Y}_t)$ for nominal):
...
To derive the asymptotic distributions of the sample measures we first need a central limit theorem for the vectors $\hat{\mathbf p}$ and $\hat{\mathbf f}$. Because the observations are not assumed to dependent, we instead assume that $(X_t)$ is strictly stationary and $\alpha$-mixing with exponetially decreasing weights. \\
To define $\alpha$-mixing (see Bradley, 2005), let  
\[
\mathcal{F}_J^L := \sigma(X_t : J \le t \le L),
\]
where $\sigma(\cdot)$ denotes the $\sigma$-field generated by the enclosed random variables.
The $\alpha$-mixing coefficients of the process $(X_t)_{t \in \mathbb{Z}}$ are given by
\[
\alpha(n)
:= \sup_{j \in \mathbb{Z}}
\ \sup_{A \in \mathcal{F}_{-\infty}^j,\; B \in \mathcal{F}_{j+n}^{\infty}}
\bigl| P(A \cap B) - P(A)P(B) \bigr|.
\]

A process is called \emph{$\alpha$-mixing} (or strongly mixing) if  
\[
\alpha(n) \longrightarrow 0 \qquad \text{as } n \to \infty.
\]

Furthermore, the process is assumed to be \emph{$\alpha$-mixing with exponentially decaying coefficients}, meaning that there exist constants $C>0$ and $\rho \in (0,1)$ such that
\[
\alpha(n) \leq C \rho^n \qquad \text{for all } n \in \mathbb{N}.
\]
This condition is sufficient to ensure the summability requirement
\[
...
&\le \sum_{n=1}^{\infty} (C \rho^n)^{\lambda/(2+\lambda)} 
= C^{\lambda/(2+\lambda)} \sum_{n=1}^{\infty} \rho^{\,n\lambda/(2+\lambda)} \\
&= C^{\lambda/(2+\lambda)} \left(\overbrace{\sum_{n=0}^{\infty} \rho^{n \lambda/2+\lambda}}^{\text{geom. series*}} -1 \right) 
= C^{\lambda/(2+\lambda)}\left(\frac{1}{1-\rho^{\lambda/2+\lambda}} - 1\right) \tag*{Since $0 < \rho^{\lambda/(2+\lambda)}< 1,  ~\forall \lambda$}   
< \infty,
\end{align*}

Since $g_i(X_t):= I_{X_t \leq s_i} = Z_{t,i}$ (as well as $h_i(X_t):= I_{X_t = s_i} = Y_{t,i}$ for nominal data) is a measurable function of $X_t$, both strict stationarity as well as $\alpha$-mixing  are presevered in $\boldsymbol{Z} $(/$\boldsymbol{Y}$; ich konnte bisher leider noch keinen Verweis in der Literatur finden der das explizit macht). 

Moreover, since $\boldsymbol{Z_t}$ and $\boldsymbol{Y_t}$ are vectors of indicatorvariables we trivially also have
\[E\lvert Z_{t,i}\rvert^{2+\lambda} < \infty  \text{ for any } \lambda > 0\].

so that the CLT from Ibragimov (Theorem 1.7; 1962) applies to $\boldsymbol{Z_t}$ and $\boldsymbol{Y_t}$. It follows: \\
