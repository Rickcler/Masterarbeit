...
\]
are measurable functions of $X_t$ (see Bauer and Burckel, 1995, p.~49).
...
$2+\lambda$. This condition is trivially satisfied in our setting, since
$\boldsymbol{Z}_t$ and $\boldsymbol{Y}_t$ are vectors of indicator variables.
In particular, for all $i$ and any $\lambda > 0$,
\[
\mathbb{E}\lvert Z_{t,i}\rvert^{2+\lambda} < \infty,
\]
and analogously for $Y_{t,i}$.

According to the Cramer-Wold device, a series of random vectors $\boldsymbol{Z}_T \in \mathbb{R}^{m+1}$ assympotatically approaches a multivariat normal distribution exactly if for every $\boldsymbol{a} \in \mathbb{R}^{m+1}$ the scalar projections 
\[\boldsymbol{a}^{\top}\boldsymbol{Z}_t = \sum_{i=0}^{m}a_i Z_{t,i}\]
converge to a univariate normal distribution.
\end{document}
