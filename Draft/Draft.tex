...
Since $U_t$ is strictly stationary, $\alpha$-mixing and satsifying the summability aswell as the moement condition, the CLT from Ibragimov is applicable. Hence, according to the Cramer-Wold device, the sample mean
\[\bar{\boldsymbol{Z}_n}:= \frac{1}{n} \sum_{t=1}^{n} \boldsymbol{Z}_t\]
satisfies the following multivariat CLT: 
\[\sqrt{n}\bigl(\bar{\boldsymbol{Z}_n}- \boldsymbol{\mu}_z\bigr) \xrightarrow[n \to \infty]{d} \mathcal{N}\bigl(\boldsymbol{0}, \boldsymbol{\Sigma}_Z\bigr)\]
\end{document}
